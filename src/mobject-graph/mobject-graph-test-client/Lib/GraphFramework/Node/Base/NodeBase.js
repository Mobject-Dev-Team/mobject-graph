...
class NodeBase extends LGraphNode {
    ...
    addProperty(property_name, default_value, type, options) {
        ...
        options.iecDataType = options.iecDataType || LiteGraph.getIecDatatype(type) || {};

        if (options.iecDataType && options.iecDataType.type == 'ENUM' && typeof default_value == 'string' && options.iecDataType.findEnumEntryByString(default_value))
            default_value = options.iecDataType.findEnumEntryByString(default_value).value;

        let visualWidget = LiteGraph.getVisualWidgetByType(type);
        let property = this._addProperty(property_name, default_value, type, options);
    ...
    addContent(content_name, default_value, type, options) {
        ...
        options.label = typeof options.label === "undefined" ? content_name : options.label;
        options.defaultValue = typeof options.defaultValue === "undefined" ? default_value : options.defaultValue;

        let widget = new visualWidget(this, null, options);

        this.registerStatusHandler(function (status, node) {
            let content = status.content;
            widget.changeValue(content.find(element => element.name === content_name).data, node);
        });
    ...
    getConnectionPos(is_input, slot_number, out) {
        ...
        if (is_input && this.inputs && this.inputs[slot_number] && (typeof this.inputs[slot_number].widget_slot !== 'undefined')) {
        ...
        if (is_input) {
            ...
            for (var i = slot_number - 1; i > 0; i--) {
                if (is_input && this.inputs && this.inputs[i] && (typeof this.inputs[i].widget_slot !== 'undefined')) {
                        actual_slot_pos--;
                }
            }
    ...
    onConnectionsChange(input_type, slot, flag, link,x) {
        ...
        if (input_type != LiteGraph.INPUT)
            return;

        if (typeof this.inputs[slot].widget_slot === 'undefined')
            return;
